...
namespace formula.f1989
{
	...
	*/
	public class Particle:AbstractParticle
	{
		...
		//UPGRADE_NOTE: The initialization of  'qnC' was moved to static method 'formula.f1989.Particle'. 'ms-help://MS.VSCC.2003/commoner/redir/redirect.htm?keyword="jlca1005"'
		...
		*/
		protected internal override int getQuantumNumber(bool timeHelicity)
		{
			...
			if (timeHelicity)
			{
				...
				return MathL.trunc((1.0 / 2.0) * (((P - (2 * x) + 2) * (1 - (kappa * Q * (2 - k)))) + (k - 1 - ((1 + kappa) * Q * (2 - k))) + C));
			}
			else
			{
		...
		private int[] calculateEquationsK()
		{
			...
			W[0] = Wrap(); //NOTE: This has been altered
