...
namespace formula.f1989
{
	...
	/*
	* Implementation of 1989 formula.
	*/
	public class Formula:AbstractFormula
	{
		...
		override public formula.SelfCouplingFunction SelfCouplingFunction
		{
			get
			{
				...
				return formula.f1989.SelfCouplingFunction.Instance;
			}
		...
		*/
		public override double M(AbstractParticle particle)
		{
			...
			return (Constants.massElement * Constants.alphaPlus * ((MainFunctions.G(particle) + MainFunctions.S(particle) + MainFunctions.F(particle) + MainFunctions.phi(particle)) + (4 * particle.q * Constants.alphaMinus)));
		}
	}
}