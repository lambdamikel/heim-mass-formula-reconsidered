...
namespace formula.HeimGroup
{
	...
	/*
	* Implementation of Heim Group formula (based on their code).
	*/
	public class Formula:AbstractFormula
	{
		...
		override public formula.SelfCouplingFunction SelfCouplingFunction
		{
			get
			{
				return formula.HeimGroup.SelfCouplingFunction.Instance;
			}
		...
		{
			/*
			* In the heim group code they are simply using the 
			* parameters on p2 of Selected Results as input.
			* (This means they are using the 1982 equation for q)
			*/
			try
			{
		...
		/*
		* The Mass Equation, Heim Group Code
		*/
		public override double M(AbstractParticle particle)
		{
			double amu = 0.225899134227831 * System.Math.Pow(10, - 30);
			
			// Result of mass equation (in kg)
			return ((Constants.massElementHG * (MainFunctions.G(particle) + MainFunctions.S(particle) + MainFunctions.F(particle) + MainFunctions.phi(particle)) * Constants.alphaPlus) + (Constants.massElementHG * 4 * particle.q * Constants.alphaMinus));
		}
	}
}